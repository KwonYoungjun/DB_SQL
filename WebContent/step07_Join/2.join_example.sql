...
-- ANSI join(표준) (오라클이나 데이터베이스에 상관없이 표준으로 쓸수있음)
select s.name, deptno1, d.dname
from STUDENT s join DEPARTMENT d --join 이라는 단어가 들어감
on s.deptno1=d.deptno; -- on 뒤에 조건식

--[문제1]학생테이블(student)과 교수테이블(professor) 조인
-- 이름(s), 학과번호(s), 지도교수번호(s), 지도교수이름(p) 출력하시오.
select * from professor;
select * from student;
select s.name, s.deptno1 as "학과번호", s.profno, p.name as "지도교수"
...
-- 지도교수이름(p)을 출력하시오.
select * from DEPARTMENT;
select s.deptno1 as "학과번호", s.name as "학생이름", p.name as "지도교수이름"
from STUDENT s, PROFESSOR p, DEPARTMENT d
where d.deptno = 101 AND s.profno = p.profno; 
...
from student s,professor p
where s.profno = p.profno(+); -- 15 + 6(NULL) = 21
