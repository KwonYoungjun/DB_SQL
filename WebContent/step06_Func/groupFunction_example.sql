...
select variance(bonus) from professor; -- variance 분산구하는 함수
select stddev(bonus) from professor; -- 표준편차 구하는 함수
select sqrt(variance(bonus)) from professor; -- 분산에 루트 = 표준편차
...
/*
 * 조건절
 * 1. select * from <테이블명> where 조건식 order by 칼럼;
 * 2. select * from <테이블명> group by 칼럼명 having 조건식 order by 칼럼;
 * 
 */
...
select * from student;
select avg(weight), avg(height), max(weight), min(weight),
max(height), min(height) from student
group by grade;

--[실습3] 몸무게 평균이 60이상인 경우만 검색
select * from student;
select avg(weight), avg(height), max(weight), min(weight), max(height), min(height) 
from student
